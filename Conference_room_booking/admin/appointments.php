...
<?php 
	...
	$doctor= $conn->query("SELECT * FROM rooms_list ");
...
<div class="container-fluid">
	<div class="col-md-12">
		<div class="card">
			<div class="card-body">
				<button class="btn-primary btn btn-sm" type="button" id="new_appointment"><i class="fa fa-plus"></i> New Booking</button>
				<br>
				<table class="table table-bordered">
					...
					if($_SESSION['login_type'] == 2)
						$where = " where room_id = ".$_SESSION['login_room_id'];
					$qry = $conn->query("SELECT * FROM bookings_list ".$where." order by id desc ");
					while($row = $qry->fetch_assoc()):
					?>
					<tr>
						<td><?php echo date("l M d, Y h:i A",strtotime($row['schedule'])) ?></td>
						<td><?php echo $doc_arr[$row['room_id']]['name']?></td>
						<td><?php echo $p_arr[$row['employee_id']]['name'] ?></td>
						<td>
							<?php if($row['status'] == 0): ?>
